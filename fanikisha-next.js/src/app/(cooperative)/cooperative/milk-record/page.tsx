...
// const CombinedMilkRecordsPage = () => {
//   const [searchTerm, setSearchTerm] = useState("");
//   const { loading, error } = useMilkRecord();
...
//       const storedRecords = JSON.parse(localStorage.getItem('allMilkRecords') || '[]');
//       const sortedRecords = storedRecords.sort((a: MilkRecord, b: MilkRecord) => 
//         new Date(b.date).getTime() - new Date(a.date).getTime()
//       );
//       return sortedRecords;
//     };

//     const initialRecords = loadMilkRecords();
...
//       setMilkRecords(prev => [newCollection, ...prev]);
//     };

//     window.addEventListener('newMilkCollection', handleNewCollection as EventListener);
    
//     return () => {
//       window.removeEventListener('newMilkCollection', handleNewCollection as EventListener);
//     };
//   }, []);

//   useEffect(() => {
//     if (selectedFarmer) {
//       const farmerCollectionsKey = `farmerCollections_${selectedFarmer.farmer_id}`;
//       const storedCollections = JSON.parse(localStorage.getItem(farmerCollectionsKey) || '[]');
//       setFarmerCollections([...storedCollections, selectedFarmer]);
//     }
//   }, [selectedFarmer]);
...
//     if (editingRecord) {
//       const updatedRecord = { ...newCollection, record_id: editingRecord.record_id };
      
//       // Update farmer-specific collections
//       const farmerCollectionsKey = `farmerCollections_${selectedFarmer?.farmer_id}`;
//       const farmerCollections = JSON.parse(localStorage.getItem(farmerCollectionsKey) || '[]');
//       const updatedFarmerCollections = farmerCollections.map((record: MilkRecord) => 
//         record.record_id === editingRecord.record_id ? updatedRecord : record
//       );
//       localStorage.setItem(farmerCollectionsKey, JSON.stringify(updatedFarmerCollections));

//       // Update global records
//       const allRecords = JSON.parse(localStorage.getItem('allMilkRecords') || '[]');
//       const updatedAllRecords = allRecords.map((record: MilkRecord) => 
//         record.record_id === editingRecord.record_id ? updatedRecord : record
//       );
//       localStorage.setItem('allMilkRecords', JSON.stringify(updatedAllRecords));

//       setFarmerCollections(updatedFarmerCollections);
//       setMilkRecords(updatedAllRecords);
//     } else {
//       const newRecordId = Date.now();
//       const newRecord = { ...newCollection, record_id: newRecordId };

//       // Add to farmer-specific collections
//       const farmerCollectionsKey = `farmerCollections_${selectedFarmer?.farmer_id}`;
//       const farmerCollections = JSON.parse(localStorage.getItem(farmerCollectionsKey) || '[]');
//       localStorage.setItem(farmerCollectionsKey, JSON.stringify([...farmerCollections, newRecord]));

//       // Add to global records
//       const allRecords = JSON.parse(localStorage.getItem('allMilkRecords') || '[]');
//       localStorage.setItem('allMilkRecords', JSON.stringify([...allRecords, newRecord]));

//       setFarmerCollections(prev => [...prev, newRecord]);
...
//   const filteredRecords = milkRecords.filter((record: MilkRecord) => {
//     const total = (record.milk_quantity * record.price).toFixed(2);
//     const recordDate = new Date(record.date).toLocaleDateString();
//     return (
//       record.first_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
...
//   // Rest of the JSX remains the same as in the original code
//   return (
//     <Layout>
//       <div className="bg-white">
//         {!selectedFarmer ? (
...
//                       {editingRecord ? "Edit Milk Collection" : "Add Milk Collection"}
//                     </h2>
//                     <button onClick={resetForm} className="text-blue-500 text-xl">X</button>
//                   </div>
//                   <form onSubmit={handleSubmit}>
//                     <input
//                       type="date"
...
//                         {editingRecord ? "Update" : "Save"}
//                       </button>
//                       <button type="button" onClick={resetForm} className="bg-red-500 text-white px-4 py-2 rounded ml-2">
//                         CancelCancel</button>
//                     </div>
//                   </form>
//                 </div>
