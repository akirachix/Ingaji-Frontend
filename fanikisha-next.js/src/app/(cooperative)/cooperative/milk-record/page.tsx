...
// import { fetchMilkRecordByFarmerId } from "@/app/utils/fetchMilkRecords";
...
//         ...initialRecords.map(record => ({ ...record, is_collection: false })),
//         ...storedRecords,
//       ];
...
//           case 'update':
//             return prevRecords.map(r => (r.record_id === record.record_id ? record : r));
//           case 'delete':
//             return prevRecords.filter(r => r.record_id !== record.record_id);
//           default:
//             return prevRecords;
//         }
//       });
...
//   useEffect(() => {
//     if (selectedFarmer) {
//       const farmerRecords = records.filter(record =>
...
//   const filteredRecords = records.filter(record => !record.is_collection && (
//     record.first_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
//     record.last_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
//     record.milk_quantity.toString().includes(searchTerm) ||
//     record.price.toString().includes(searchTerm) ||
//     new Date(record.date).toLocaleDateString().includes(searchTerm) ||
//     (record.milk_quantity * record.price).toFixed(2).includes(searchTerm)
//   ));
...
//       ? records.map(record => record.record_id === editingRecord.record_id ? updatedRecord : record)
//       : [...records, { ...updatedRecord, record_id: Math.max(...records.map(r => r.record_id), 0) + 1 }];

//     setRecords(updatedRecords);
//     localStorage.setItem('allMilkRecords', JSON.stringify(updatedRecords));

//     window.dispatchEvent(new CustomEvent('milkRecordUpdated', {
//       detail: { type: editingRecord ? 'update' : 'add', record: updatedRecord }
//     }));
...
//                 )}
//                 <div className="mt-4">
//                   <button onClick={() => setShowForm(true)} className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition">
//                     <IoAddCircleOutline className="inline-block mr-2" /> Add Collection
//                   </button>
//                 </div>
//               </section>
...
//                     <button
//                       key={index}
//                       className={`mx-1 py-1 px-3 rounded ${currentPage === index + 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'}`}
//                       onClick={() => setCurrentPage(index + 1)}
//                     >
//                       {index + 1}
//                     </button>
...
//             <button onClick={() => setSelectedFarmer(null)} className="flex items-center text-blue-500 mb-4">
//               <IoArrowBack className="mr-2" /> Back to Records
//             </button>
//             <h2 className="text-3xl font-bold mb-4">{selectedFarmer.first_name} {selectedFarmer.last_name}&apos;s Collections</h2>
//             <div className="overflow-x-auto">
//               <table className="min-w-full bg-white border border-gray-200 shadow-md rounded-lg">
//                 <thead>
//                   <tr className="bg-gray-50 uppercase text-xs leading-normal tracking-wider border-b border-gray-200">
...
//                       <td className="py-3 px-6">{collection.milk_quantity}</td>
//                       <td className="py-3 px-6">{collection.price}</td>
//                       <td className="py-3 px-6">{new Date(collection.date).toLocaleDateString()}</td>
//                       <td className="py-3 px-6">{(collection.milk_quantity * collection.price).toFixed(2)}</td>
//                       <td className="py-3 px-6">
//                         <button onClick={() => handleEdit(collection)} className="text-blue-500 hover:underline">
//                           <IoPencil /> Edit
//                         </button>
//                       </td>
//                     </tr>
...
//             <h2 className="text-2xl font-bold mb-4">{editingRecord ? 'Edit Collection' : 'Add Collection'}</h2>
//             <form onSubmit={handleSubmit}>
//               <div className="mb-4">
//                 <label className="block mb-1" htmlFor="milk_quantity">Quantity (L)</label>
//                 <input
//                   type="number"
//                   name="milk_quantity"
//                   value={newCollection.milk_quantity}
...
//               </div>
//               <div className="mb-4">
//                 <label className="block mb-1" htmlFor="price">Price per Litre</label>
//                 <input
//                   type="number"
//                   name="price"
...
//                 <button type="button" onClick={resetForm} className="bg-gray-300 text-gray-700 py-2 px-4 rounded">
//                   Cancel
//                 </button>
