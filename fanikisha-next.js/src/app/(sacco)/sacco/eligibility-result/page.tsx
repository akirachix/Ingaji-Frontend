...
const EligibilityResultPage: React.FC = () => {
  const [eligibilityResult, setEligibilityResult] = useState<EligibilityResult | null>(null);
  const router = useRouter();

  useEffect(() => {
    // Retrieve eligibility result from localStorage
    const storedResult = localStorage.getItem('eligibilityResult');
    if (storedResult) {
      setEligibilityResult(JSON.parse(storedResult));
    } else {
      // If no result is found, navigate back to the form page
      router.push('/');
    }
  }, [router]);
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-4">Eligibility Result</h1>
      <div className="bg-white shadow-md rounded-lg p-6">
        <p className="text-xl">
          {eligibilityResult.isEligible ? (
            <span className="text-green-500">Congratulations! You are eligible for a loan.</span>
          ) : (
            <span className="text-red-500">Sorry, you are not eligible for a loan.</span>
          )}
        </p>
        <p className="text-lg mt-4">
          Probability of eligibility: <span className="font-semibold">{(eligibilityResult.probability * 100).toFixed(2)}%</span>
        </p>
      </div>
