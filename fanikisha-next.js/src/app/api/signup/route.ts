...
export async function POST(request: NextRequest) {
  ...
  try {
    const { first_name, last_name, username, email,role, password, } = await request.json();
    const response = await fetch(`${baseUrl}/api/signup/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ first_name, last_name, username,role, email, password }),
    });
    ...
    return NextResponse.json({data: result, message: 'Signup successful! '},{ status: 200 });
  }
  catch (error) {
    return NextResponse.json(
