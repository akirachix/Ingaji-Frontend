...
import { useFetchCooperatives } from "../hooks/useFetchCooperative";
import { FarmerData } from "../utils/types";

const CooperativeDashboard: React.FC = () => {
  const { data: cooperatives, isLoading: loadingCooperatives, error: errorCooperatives } = useFetchCooperatives();
    ...
    const fetchFarmersData = async () => {
      if (cooperatives && cooperatives.length > 0) {
        ...
        for (const cooperative of cooperatives) {
          try {
            const response = await fetch(`https://fanikisha-3beb7fcefffe.herokuapp.com/api/farmers/?cooperative_id=${cooperative.cooperative_id}`);
            if (!response.ok) {
                  ...
                  <table className="min-w-full bg-white border border-gray-200 shadow-md rounded-lg">
                    ...
                    <tbody>
                      {currentCooperatives.map(cooperative => {
                        ...
                        return (
                          <tr key={cooperative.cooperative_id} className="border-b border-gray-200 hover:bg-gray-100 transition duration-200">
