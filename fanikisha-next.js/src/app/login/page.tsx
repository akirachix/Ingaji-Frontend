  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        // Assuming 'data' contains the role of the user
        const { role, token } = data; // Destructure role and token from response
        ...
        if (role === "sacco") {
          router.push("/sacco/Overview"); // Redirect to Sacco dashboard
        } else if (role === "cooperative") {
          router.push("/cooperative/Overview"); // Redirect to Cooperative dashboard
        } 
      } catch (err) {
        console.error("Login error:", err);
        setLoginError("Login failed. Please check your credentials.");
