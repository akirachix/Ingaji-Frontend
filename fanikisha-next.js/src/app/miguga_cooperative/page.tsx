...
const MugugaCooperative: React.FC = () => {
  ...
  if (loading) return <Layout><div className="text-customBlue text-lg lg:text-xl 2xl:text-2xl">Loading credit scores...</div></Layout>;
  if (error) return <Layout><div className="text-customBlue text-lg lg:text-xl 2xl:text-2xl">{error}</div></Layout>;
      ...
      <div className="bg-white">
        <header className="text-[#299acf] p-4">
          <div className="container mx-auto px-4 lg:px-6 2xl:px-8">
            <h1 className="text-3xl lg:text-4xl 2xl:text-5xl font-bold">Miguga Cooperative</h1>
          </div>
        </header>
        <main className="container mx-auto px-4 lg:px-6 2xl:px-8">
          <section className="mt-4">
            ...
            {filteredScores.length === 0 ? (
              <p className="text-customBlue text-lg lg:text-xl 2xl:text-2xl">No credit scores data available.</p>
            ) : (
              <>
                    ...
                    <table className="min-w-full bg-white border border-gray-200 shadow-md rounded-lg">
                      <thead>
                        <tr className="bg-gray-50 uppercase text-xs lg:text-sm leading-normal tracking-wider border-b border-gray-200">
                          <th className="py-2 lg:py-3 px-3 lg:px-4 text-left font-bold text-customBlue">Farmer ID</th>
                          <th className="py-2 lg:py-3 px-3 lg:px-4 text-left font-bold text-customBlue">Last Checked</th>
                          <th className="py-2 lg:py-3 px-3 lg:px-4 text-left font-bold text-customBlue">Score</th>
                          <th className="py-2 lg:py-3 px-3 lg:px-4 text-left font-bold text-customBlue">Status</th>
                          <th className="py-2 lg:py-3 px-3 lg:px-4 text-left font-bold text-customBlue">Eligibility</th>
                        </tr>
                      </thead>
                      <tbody>
                        {currentScores.map((score: CreditScore) => (
                          <tr key={score.credit_score_id} className="border-b border-gray-200 hover:bg-gray-100 transition duration-200">
                            <td className="py-2 lg:py-3 px-3 lg:px-4 text-sm lg:text-base">{score.farmer_id}</td>
                            <td className="py-2 lg:py-3 px-3 lg:px-4 text-sm lg:text-base">{score.last_checked_date}</td>
                            <td className="py-2 lg:py-3 px-3 lg:px-4 text-sm lg:text-base">{score.score}</td>
                            <td className="py-2 lg:py-3 px-3 lg:px-4 text-sm lg:text-base">
                              <span className={score.is_eligible ? "text-green-500" : "text-red-500"}>
