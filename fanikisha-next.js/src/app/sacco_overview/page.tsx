...
//       const farmers = (farmersData as Farmer[]).filter((farmer) =>
//         format(new Date(farmer.created_at), 'yyyy-MM') === selectedMonth
//       );
...
//           farmer.cooperative_number === `C/2024/${coop.cooperative_id}` && farmer.is_eligible 
//         );
...
//       const topCooperatives = Object.entries(cooperativeFarmerCounts)
//         .map(([name, value]) => ({ name, value }))
//         .sort((a, b) => b.value - a.value)
//         .slice(0, 4);

//       setBarData(topCooperatives);
//     }
...
const Overview: React.FC = () => {
  ...
  useEffect(() => {
    if (cooperativesData && loanEligibilityData && farmersData) {
      ...
      const creditScoresData = Array.isArray(loanEligibilityData) 
        ? (loanEligibilityData as CreditScore[]).filter((score) =>
            format(new Date(score.last_checked_date), 'yyyy-MM') === selectedMonth
          )
        : [];

      const farmers = Array.isArray(farmersData) 
        ? (farmersData as Farmer[]).filter((farmer) =>
            format(new Date(farmer.created_at), 'yyyy-MM') === selectedMonth
          )
        : [];

      const cooperatives = Array.isArray(cooperativesData) ? cooperativesData as Cooperative[] : [];
